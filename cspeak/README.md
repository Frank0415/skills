...
2. **It ships a real Simplified Chinese technique layer（简体中文层）.**
   ...
   problems Chinese text actually has（欧化长句、公文腔、成语堆叠、中英夹杂、
   被字句滥用）, not a translation of the English rules.
...
| 「相关费用之核销单据应尽速填妥并缴交，俾利后续作业之进行。」 | 「请在每月 25 日前把核销单交给财务部的王小姐。我们收到当天就开始处理。」 |
...
| 「请先 review 这份 proposal 的 scope，确认 stakeholder 的 alignment 没问题之后，我们再 kick off 这个 project。」 | 「请先看过这份提案的范围，确认相关的人都同意了，我们再开始做。」 |
...
references/chinese-techniques.md  中文手法（以中文撰写）
```
...
「列出改了什么」 to see the per-change principle table.
...
bureaucratic. Works on English and Simplified Chinese; the principles apply
