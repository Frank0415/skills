...
除非我明确要求“批量处理多个 PDF”,不得遍历目录或上传列表并一次性为多个 PDF 生成讲义;一次任务只选定一个目标 PDF。
3. 读取并记录:
- 文件名;
- 课程名称;
- lecture / chapter 标题;
...
Python 或 shell 脚本可以用于当前目标 PDF 的渲染、文本提取、图片压缩、base64 和覆盖检查,但不得默认批量遍历多个 PDF 并生成多个最终 HTML。
