...
Write the HTML, navigation, captions, explanations, QA report, and final response in Chinese. In authored text, write every proper name and technical term in both Chinese and English, including repeated occurrences, headings, navigation, captions, translations, explanations, and summaries. Use `中文名称（English name）`, for example `全归约（All-Reduce）` and `随机梯度下降（Stochastic Gradient Descent, SGD）`. Keep each bilingual pairing consistent throughout the guide. Use established Chinese names; when none is established, provide a faithful Chinese descriptive translation alongside the exact English name without presenting it as an official translation. Explain each term's lecture-specific meaning where needed. Preserve original slide images and source-title quotations; keep formulas, variables, code, API identifiers, filenames, paths, and commands exact, adding bilingual explanations in surrounding prose rather than rewriting those literals.

Write direct, source-specific Chinese teaching prose. Remove filler and repetition while preserving source claims, necessary reasoning, technical terms, and meaningful uncertainty. When prose needs targeted revision for formulaic phrasing or unclear explanations, read [Chinese teaching prose guidance](references/chinese-teaching-prose.md). The reference supports the course's technical register and output contract; routine conversion does not require loading it.
...
7. Explanations are specific to the source content rather than copied OCR or template language, and every proper name and technical term in authored prose has its Chinese and English forms.
